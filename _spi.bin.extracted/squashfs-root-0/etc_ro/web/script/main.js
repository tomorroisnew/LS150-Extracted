    ...
    for(var  i=0  ;  i<str.length;  i++  )
   	{
   		 if( (str.charCodeAt(i)== 34) || (str.charCodeAt(i) == 39) || (str.charCodeAt(i) == 44)
   		 	|| (str.charCodeAt(i)== 60) || (str.charCodeAt(i)== 62) || (str.charCodeAt(i)== 92) || (str.charCodeAt(i)== 32) )
   		 {
...
function onStatusLoad () {
	var func = function(){
		...
		$.getJSON("httpapi.asp?command=getStatusEx", function(json){
			...
			document.getElementById("firmVersionValue").innerHTML = json.build + json.firmware;
			...
			if(json.VersionUpdate == "1")
			{
		 			...
		 			elem.innerHTML=i18n_alert[window.langindex]["online.upgrade.confirm.a"] + " " + json.NewVer;
			}
			
			var myselect_old = $("#languageSelect");
			myselect_old.selectmenu('refresh');
...
//	else
//	{
		if(!(a.wifi.level == "OPEN" && a.wifi.secur != "WEP")){
		/*
...
function showWiFiList(req){
	...
	for(i=0;i<dataObj.aplist.length;i++)
	{
				...
				if(a != null)
				{
					...
					if(wifi.level == "OPEN" && wifi.secur != "WEP")
						;
					else
						pox=pox+225;
...
function funRestoreToDefaultReturn(req)
{
	...
	elem.innerHTML="Firmware is broken. This is backup image, please update your firmware first!";
...
$("#div_online_update").hide();
	...
	if( document.UploadFirmware.filename.value.indexOf("jffs2.img", 0) != -1 ||
		document.UploadFirmware.filename.value.indexOf("jffs2_user2.img", 0) != -1 ||
		document.UploadFirmware.filename.value.indexOf("mcu.bin", 0) != -1)
	{
		$('#popupDialog_burningFirmware').popup('open');
		$("#popupDialog_burningFirmware_text").text(i18n_alert[window.langindex]["system.complete_reboot"]);			
	}
	else if(document.UploadFirmware.filename.value.indexOf("all_burning.img", 0) != -1)
	{
		//$('#popupDialog_burningFirmware').popup('open');
		//$("#popupDialog_burningFirmware_text").text("Please wait at least 2 minutes until the device reboots itself");			
